...
resource "intersight_boot_precision_policy" "boot_policy_for_iscsi_boot_from_san" {
  ...
  boot_devices = var.boot_mode == "Uefi" ? [{ enabled = true, class_id = "boot.VirtualMedia", name = "kvm-mapped-dvd", object_type = "boot.VirtualMedia", additional_properties = jsonencode({ Subtype = "kvm-mapped-dvd" }) },
    { enabled = true, class_id = "boot.Iscsi", name = "iscsi-a", interface_name = "iscsi-a", object_type = "boot.Iscsi", additional_properties = jsonencode({ port = 0, interfacename = "iscsi-a", bootloader = { description = "iscsi", name = "bootx64.efi", path = "\\EFI\\BOOT\\BOOTx64.EFI" } }) },
    { enabled = true, class_id = "boot.Iscsi", name = "iscsi-b", interface_name = "iscsi-b", object_type = "boot.Iscsi", additional_properties = jsonencode({ port = 0, interfacename = "iscsi-b", bootloader = { description = "iscsi", name = "bootx64.efi", path = "\\EFI\\BOOT\\BOOTx64.EFI" } }) }] : [{ enabled = true, class_id = "boot.VirtualMedia", name = "kvm-mapped-dvd", object_type = "boot.VirtualMedia", additional_properties = jsonencode({ Subtype = "kvm-mapped-dvd" }) },
...
#VNIC: 02-VDS0-A
resource "intersight_vnic_eth_if" "vnic02-VDS0-A" {
  name             = "02-VDS0-A"
  failover_enabled = false
  mac_address_type = "POOL"
