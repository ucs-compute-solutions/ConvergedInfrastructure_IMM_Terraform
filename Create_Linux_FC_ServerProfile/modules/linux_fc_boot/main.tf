...
resource "intersight_boot_precision_policy" "boot_policy_for_fc_boot_from_san" {
  ...
  boot_devices             = var.boot_mode == "Uefi" ? [
    ...
    {enabled=true, class_id="boot.San", name=var.san_boot_device_1_name,interface_name = "vHBA-A",object_type="boot.San", additional_properties =jsonencode({wwpn=var.san_boot_target_1_wwpn, lun=var.lun_id, interfacename = "vHBA-A", bootloader={description="rhel",name="bootx64.efi",path="\\EFI\\BOOT\\BOOTx64.EFI"}})},

    {enabled=true, class_id="boot.San", name=var.san_boot_device_2_name,interface_name = "vHBA-A",object_type="boot.San", additional_properties =jsonencode({wwpn=var.san_boot_target_1_wwpn, lun=var.lun_id, interfacename = "vHBA-A", bootloader={description="rhel",name="bootx64.efi",path="\\EFI\\BOOT\\BOOTx64.EFI"}})},

    {enabled=true, class_id="boot.San", name=var.san_boot_device_3_name,interface_name = "vHBA-B",object_type="boot.San", additional_properties =jsonencode({wwpn=var.san_boot_target_1_wwpn, lun=var.lun_id, interfacename = "vHBA-B", bootloader={description="rhel",name="bootx64.efi",path="\\EFI\\BOOT\\BOOTx64.EFI"}})},

    {enabled=true, class_id="boot.San", name=var.san_boot_device_4_name,interface_name = "vHBA-B",object_type="boot.San", additional_properties =jsonencode({wwpn=var.san_boot_target_1_wwpn, lun=var.lun_id, interfacename = "vHBA-B", bootloader={description="rhel",name="bootx64.efi",path="\\EFI\\BOOT\\BOOTx64.EFI"}})}] : [
